...
def solucao_inicial(tam):
	sol = [str(randint(0,1)) for i in range(tam)] #Aqui cria-se aleatoriamente um número binário (Dados)
	return ''.join(sol)

def avaliacao(sol):
	return sol.count('1') #Aqui se é instaurado um método de avaliação, quanto mais '1's maior a nota

def gerar_vizinhos(sol):
	vizinhos = []  #Cria uma lista para gerar todos os vizinhos da solução inicial
	for i in range(len(sol)):
		lista_bits = list(sol)
		lista_bits[i] = '1' if sol[i] == '0' else '0' #Crio um conjunto de vizinhos ao trocar os números da solução inicial
		vizinho = ''.join(lista_bits) #guardo os vizinhos na lista
		vizinhos.append(vizinho)
	return vizinhos

def movimento_tabu(sol, vizinho):
	for i in range(len(sol)):
		if sol[i] != vizinho[i]: #Detecção de movimento proibido
			return i
...
def obter_melhor_vizinho(vizinhos, lista_tabu, melhor_sol, sol):
	...
	for vizinho in vizinhos:
		if movimento_tabu(sol, vizinho) in lista_tabu:
			if avaliacao(vizinho) > avaliacao(melhor_sol): # verifica se a avaliação do melehor vizinho é melhoer que a solução inicial
				return vizinho
		else:
			return vizinho
...
if __name__ == '__main__':
	melhor_solucao = busca_tabu(bits=4, BTmax=2, T=2)
